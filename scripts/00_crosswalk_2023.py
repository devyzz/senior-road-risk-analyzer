...
# TM 좌표 기반 산점도 시각화 (좌표 변환 없이 확인용)
print("📊 TM 좌표 기반 산점도 시각화...")
raw_coords = df_signal[df_signal['X좌표'].notna() & df_signal['Y좌표'].notna()]  # TM 좌표가 유효한 행 기준

plt.figure(figsize=(8, 8))
plt.scatter(raw_coords['X좌표'], raw_coords['Y좌표'], s=1, c='blue')
plt.title("서울시 TM 좌표 기반 횡단보도 위치")
plt.xlabel("X좌표")
plt.ylabel("Y좌표")
plt.grid(True)
...
# 최종 출력용 데이터 정리
df_signal = df_signal[['순번', '자치구', '횡단보도종류', '위도', '경도', '주소']]
...
# 신호등 위치 마커 추가
valid_rows = df_signal[df_signal['위도'].notna() & df_signal['경도'].notna()]

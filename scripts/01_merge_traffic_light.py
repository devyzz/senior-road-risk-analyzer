...
#        신호등 존재 여부를 판별하여, Boolean(1(True)/0(False)) 값으로 데이터 추가
#        ▶ near_traffic_light
# ──────────────────────────────────────────────────────────────
...
def mark_zone_proximity(accident_df, trafficlight_df, radius_m=10, buffer_m =10):
    col_name = "near_traffic_light"
    accident_df[col_name] = 0
    
    trafficlight_df = trafficlight_df.dropna(subset=["위도", "경도"])
    zone_coords_deg = trafficlight_df[["위도", "경도"]].values
    zone_coords_rad = latlng_to_radians(trafficlight_df, "위도", "경도")
...
def process_accident_year(year):
    ...
    trafficlight_path = "./data/external/trafficlight_2023.csv"
    accident_path = f"./data/raw/all_accident_info_{year}.csv"
    output_path = f"./data/processed/accident_data_with_trafficlight_{year}.csv"

    accident_df = pd.read_csv(accident_path)
    trafficlight_df = pd.read_csv(trafficlight_path)

    accident_df = mark_zone_proximity(accident_df, trafficlight_df)
